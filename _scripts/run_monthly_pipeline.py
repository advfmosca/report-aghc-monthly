#!/usr/bin/env python3
"""run_monthly_pipeline.py — Pipeline completa fase 1 (xlsx + HTML + push).

Input: --year YYYY --month MM --raw-data <path/al/JSON/dati/raw/da/Windsor>
       --senape-root <path/Desktop/SENAPE/Report AGHC>
       --repo-root   <path/del/checkout/local/del/repo>

Output:
  1. Stima reach YoY mancante (idempotente, in-place sul JSON dati)
  2. Genera xlsx in <senape-root>/<Mese Anno>/Report AGHC - KPI <Mese Anno>.xlsx
  3. Post-processa xlsx (ⓘ stimata + 1° mese live)
  4. Copia JSON in <repo>/_data/data-YYYY-MM.json
  5. Genera HTML statico in <repo>/<mese>-<anno>.html + aggiorna index.html
  6. git add + commit + push
  7. Stampa su stdout (per Claude → Slack) i 3 link finali
...
def main():
    ...
    p.add_argument("--raw-data", required=True, help="JSON dati Windsor pre-aggregati (schema data_YYYY_MM.json)")
    p.add_argument("--senape-root", required=True, help="es. /sessions/.../mnt/Report AGHC")
    ...
    # 1. Stima reach (idempotente — modifica in-place il JSON dati)
    print(f"\n[1/6] Stima reach YoY su {raw_path.name}…")
    ...
    print(f"\n[4/6] JSON copiato in repo: {repo_data_path.relative_to(repo_root)}")

    # 5. Genera HTML statico + aggiorna index
    print(f"\n[5/6] Genero HTML statico…")
    ...
    # 6. git add + commit + push
    if args.skip_push:
        print("\n[6/6] --skip-push attivo, niente push.")
    else:
        print(f"\n[6/6] Commit + push…")
        # Setup credential helper se non già fatto (idempotente)
        ...
        if not status:
            ...
            print(f"✔ Push completato — GitHub Pages aggiorna in ~30-60s")

    # 7. Output finale (per Claude → Slack)
    public_url = f"{REPO_URL}/{slug}"
    archive_url = f"{REPO_URL}/"
    computer_url = f"computer://{xlsx_path}"
    print(f"\n========================================")
    print(f"PIPELINE_DONE")
    print(f"PERIOD={period_label}")
    print(f"XLSX_PATH={xlsx_path}")
    print(f"XLSX_LINK={computer_url}")
